...
buildings = ['AGRICULTURE', 'ALLEN', 'ANIMAL SCIENCE', 'ARMES', 'ARCHITECTURE', 'Art Lab', 'Music', 'DUFF ROBLIN', 'EDUCATION', 'ELLIS', 'FLETCHER ARGUE', 'FRANK KENNEDY', 'MACHRAY HALL', 'MACHRAY', 'Distance and On-Line Education', 'HELEN GLASS', 'EITC', 'TBA', 'DRAKE', 'ST. JOHN', 'ST. PAUL', 'UNIVERSITY COLLEGE' ]
...
def get_crn_list(termcode,courseTypes):
  ...
  x = r.iter_lines()
  for line in x:
    match = re.search('^<th CL.*>([^\[\]]+) - (.+) - (.+) - (['+courseTypes+']..)</a></th>$', line)
    if match:
       crn = match.group(2)
       ...
       room = 'Not Found'
       n = get_next(x)
       while ( n is not None and not foundBldg( n ) and (not ("Section canceled." in n) ) ):
          n = get_next(x)
       #print(n)
       if n is not None:
         room = n
...
term = sys.argv[1]
courseTypes = sys.argv[2]
crns = get_crn_list(term,courseTypes);
for crn in crns:
